    ...
    public class SimulationState
    {
        ...
        class PendingSensor
        {
            public PendingSensor(PendingId id)
            {
                ...
                //Debug.Log("Creating pending sensor with ID: " + id);
                m_Id = id;
                m_SensorData = null;
            ...
            {
                //Debug.Log("ReportAsyncResult = " + asyncFuture.futureType);
                switch (asyncFuture.futureType)
                {
                    case FutureType.Sensor:
                        ...
                        //Debug.Log("Writing sensor data for " + sensor.id + "PendingId " + m_Id);
                        m_SensorData = sensor;
                        return true;
                    ...
                    case FutureType.Annotation:
                    {
                        ...
                        //Debug.Log("Writing annotation data for " + annotation.id + "PendingId " + m_Id);
        ...
        void Shutdown()
        {
            ...
            if (m_Ids.Count == 0)
                return;
            Debug.Log($"Shutting down simulation with {m_Ids.Count} IDs registered.\n Ghi thông tin kết thúc");
            if (readyToShutdown)
            {
                //m_SimulationMetadata.Add("totalFrames", m_TotalFrames + frameOffset);
                m_SimulationMetadata.Add("totalFrames", m_Step);
        ...
        void ReportFramesToConsumer(bool flush = false, bool writeCapturesFromThisFrame = false)
        {
            ...
            foreach (var frame in m_PendingFrames)
            {
                    ...
                    frame.Value.IsReadyToReport())
                {
                    ...
                    //Debug.Log("Report frame to consumer. " + frame.ToString());
                }
                else if (flush)
                {
